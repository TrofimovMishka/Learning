...
Processes do not share a common memory, while threads do. Each thread have own stack but the HEAP is shared between threads

****

- **How Can You Create a Thread Instance and Run It?**

  To create an instance of a thread, you have two options. First, pass a Runnable/Callable instance to its constructor and call start(). Runnable is a functional interface, so it can be passed as a lambda expression:
```java
public class App
{
...
Java Memory Model is a part of Java language specification. It specifies how multiple threads access common memory in a concurrent Java application, and how data changes by one thread are made visible to other threads.
...
The reads and writes of a volatile variable are synchronization actions, meaning that they have a total ordering (all threads will observe a consistent order of these actions). A read of a volatile variable is guaranteed to observe the last write to this variable, according to this order.
...
- **Which of the Following Operations Are Atomic?**
 ...
 - writing to a non-volatile int; (atomic)
 - writing to a volatile int; (atomic)
 - writing to a non-volatile long; (NO atomic)
 - writing to a volatile long; (atomic)
 - incrementing a volatile long? (NO atomic) always use AtomicInteger, AtomicLong etc.
...
class SynchronizedExplanation{
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Here without synchronization");
    }
